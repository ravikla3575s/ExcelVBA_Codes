...
Sub SetTemplateInfo(newBook As Workbook, targetYear As String, targetMonth As String)
    ...
    Dim treatmentMonth As Integer, treatmentYear as Integer, sendDate As String
    ...
    ' **請求月の計算**
    treatmentMonth = receiptMonth - 1
    If treatmentMonth <= 0 Then
        treatmentYear = receiptYear -1
        treatmentMonth = 12
    sendDate = receiptMonth & "月10日請求分"
    ...
    ' **シート名変更**
    wsTemplate.Name = "R" & (treatmentYear - 2018) & "." & treatmentMonth
    wsTemplate2.Name = ConvertToCircledNumber(treatmentMonth)

    ' **情報転記**
    wsTemplate.Range("G2").Value = treatmentYear & "年" & treatmentMonth & "月調剤分"
    wsTemplate.Range("I2").Value = sendDate
    wsTemplate.Range("J2").Value = ThisWorkbook.Sheets("設定").Range("B1").Value
    wsTemplate2.Range("H1").Value = treatmentYear & "年" & treatmentMonth & "月調剤分"
    wsTemplate2.Range("J1").Value = sendDate
    wsTemplate2.Range("L1").Value = ThisWorkbook.Sheets("設定").Range("B1").Value
End Sub
